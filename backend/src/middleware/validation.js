...
const schemas = {
    ...
    createOrder: Joi.object({
        ...
        items: Joi.alternatives().try(
            ...
            Joi.string().min(1) // Allow JSON string from FormData
        ).required(),
        total_amount: Joi.alternatives().try(Joi.number().min(0), Joi.string().pattern(/^\d+(\.\d{1,2})?$/)).required(),
        ...
        delivery_fee: Joi.alternatives().try(Joi.number().min(0), Joi.string().pattern(/^\d+(\.\d{1,2})?$/)).default(0),
