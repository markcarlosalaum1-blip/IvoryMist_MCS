...
const createOrder = async(req, res, next) => {
    try {
        ...
        console.log('Files:', req.file ? { name: req.file.originalname, size: req.file.size } : 'none');
        ...
        if (typeof itemsRaw === 'string') {
            try {
                ...
                console.log('Parsed items from JSON string:', itemsRaw);
            } catch (e) {
        ...
        console.log('Extracted data:', { customer_name, total_amount, payment_method, order_type, items: items.length });

        // Validate items array is not empty
        if (!items || items.length === 0) {
            console.error('Items validation failed:', itemsRaw);
